...
import type { UniswapV3SubgraphContext } from './sources/uniswap-v3-subgraph/types';
...
export type MeshContext = UniswapV3SubgraphContext & BaseMeshContext;
...
const importFn = (moduleId: string) => {
  ...
  switch(relativeModuleId) {
    case ".graphclient/sources/uniswap-v3-subgraph/introspectionSchema":
      return import("./sources/uniswap-v3-subgraph/introspectionSchema");
...
const uniswapV3SubgraphTransforms = [];
const additionalTypeDefs = [] as any[];
const uniswapV3SubgraphHandler = new GraphqlHandler({
              name: "uniswap-v3-subgraph",
              config: {"endpoint":"https://api.thegraph.com/subgraphs/name/uniswap/uniswap-v3"},
              baseDir,
              cache,
              pubsub,
              store: sourcesStore.child("uniswap-v3-subgraph"),
              logger: logger.child("uniswap-v3-subgraph"),
              importFn,
            });
sources[0] = {
          name: 'uniswap-v3-subgraph',
          handler: uniswapV3SubgraphHandler,
          transforms: uniswapV3SubgraphTransforms
        }
...
const merger = new(BareMerger as any)({
  ...
  return {
    ...
    get documents() {
      return [
      {
        document: TopThreePoolsDocument,
        get rawSDL() {
          return printWithCache(TopThreePoolsDocument);
        },
        location: 'TopThreePoolsDocument.graphql'
      }
    ];
    },
    fetchFn,
...
export type TopThreePoolsQueryVariables = Exact<{ [key: string]: never; }>;


export type TopThreePoolsQuery = { pools: Array<(
    Pick<Pool, 'id' | 'feeTier' | 'totalValueLockedUSD' | 'tick'>
    & { token0: Pick<Token, 'id' | 'symbol' | 'name'>, token1: Pick<Token, 'id' | 'symbol' | 'name'> }
  )> };


export const TopThreePoolsDocument = gql`
    query TopThreePools {
  pools(orderBy: totalValueLockedUSD, orderDirection: desc, first: 3, skip: 1) {
    ...
    ` as unknown as DocumentNode<TopThreePoolsQuery, TopThreePoolsQueryVariables>;


export type Requester<C = {}, E = unknown> = <R, V>(doc: DocumentNode, vars?: V, options?: C) => Promise<R> | AsyncIterable<R>
export function getSdk<C, E>(requester: Requester<C, E>) {
  return {
    TopThreePools(variables?: TopThreePoolsQueryVariables, options?: C): Promise<TopThreePoolsQuery> {
      return requester<TopThreePoolsQuery, TopThreePoolsQueryVariables>(TopThreePoolsDocument, variables, options) as Promise<TopThreePoolsQuery>;
    }
  };
}
